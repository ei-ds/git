...
function verifyConditions(pluginConfig, context) {
  ...
  if (options.prepare) {
    const preparePlugin = castArray(options.prepare).find(config => config.path && config.path === '@ei-ds/git') || {};
    ...
    pluginConfig.push = defaultTo(pluginConfig.push, preparePlugin.push);
  }
